...
async def login(req: LoginRequest):
    try:
        ...
        users = models.execute_kw(req.db, uid, req.password, 'res.users', 'read',
            [[uid]], {'fields': ['name', 'email']})
        user = users[0] if users else {}
        # API key oluştur (session yerine password kullanıyoruz)
        return {
            'success': True,
            'uid': uid,
            'name': user.get('name', ''),
            'email': user.get('email', req.email),
            'api_key': req.password,  # Odoo XML-RPC password ile çalışır
        }
...
def _merge_cards_to_form(form: dict, cards: list) -> dict:
    ...
    # Sadece görüşülen alanlara taşınmayan kartvizitler card_fields olarak dönsün
    # remaining boşsa hiçbir kartvizit kalmamış demektir — boş döndür
    card_fields = remaining[0] if remaining else {}
    return card_fields
