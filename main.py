...
# Sabit Odoo konfigurasyonu — kullanıcı bunları girmek zorunda değil
import os
ODOO_URL = os.getenv('ODOO_URL', 'https://www.mabayco.com')
ODOO_DB  = os.getenv('ODOO_DB', 'mabayco')
BACKEND_URL = os.getenv('BACKEND_URL', 'https://odooscan-backend.onrender.com')
...
async def login(req: LoginRequest):
    try:
        ...
        uid = common.authenticate(ODOO_DB, req.email, req.password, {})
        if not uid:
            return {'success': False, 'message': 'E-posta veya şifre hatalı'}
        # Kullanıcı adını ve grup bilgisini al
        models = xmlrpc.client.ServerProxy(f'{ODOO_URL.rstrip("/")}/xmlrpc/2/object')
        users = models.execute_kw(ODOO_DB, uid, req.password, 'res.users', 'read',
            [[uid]], {'fields': ['name', 'email', 'groups_id']})
        user = users[0] if users else {}

        # Yönetim/Ayarlar grubunu kontrol et
        is_admin = False
        try:
            # base.group_system XML ID'sinden grup ID'sini al
            system_group = models.execute_kw(ODOO_DB, uid, req.password, 'ir.model.data', 'search_read',
                [[['module', '=', 'base'], ['name', '=', 'group_system']]],
                {'fields': ['res_id'], 'limit': 1})
            user_group_ids = user.get('groups_id', [])
            if system_group:
                system_gid = system_group[0]['res_id']
                is_admin = system_gid in user_group_ids
            else:
                # Fallback: category_id.name ile ara (TR/EN)
                group_ids = models.execute_kw(ODOO_DB, uid, req.password, 'res.groups', 'search',
                    [[['name', 'in', ['Settings', 'Access Rights', 'Ayarlar', 'Erişim Hakları']]]],
                    {})
...
@app.post('/api/debug-groups')
async def debug_groups(req: OdooConfig):
    try:
        models, uid = get_odoo(req)
        users = models.execute_kw(req.db, uid, req.api_key, 'res.users', 'read',
            [[uid]], {'fields': ['name', 'groups_id']})
        user = users[0] if users else {}
        system_group = models.execute_kw(req.db, uid, req.api_key, 'ir.model.data', 'search_read',
            [[['module', '=', 'base'], ['name', '=', 'group_system']]],
            {'fields': ['res_id'], 'limit': 1})
        return {
            'user_groups': user.get('groups_id', []),
            'system_group': system_group,
            'is_member': system_group[0]['res_id'] in user.get('groups_id', []) if system_group else False
        }
    except Exception as e:
        return {'error': str(e)}
